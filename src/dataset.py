...
class Multimodal_Datasets(Dataset):
    def __init__(self, dataset_path, data='mosei_senti', split_type='train', if_align=False):
        ...
        # dataset {dict:3  train, test, valid}
        if not os.path.exists(dataset_path):
            print(f"  - Creating new {dataset_path} data")
        dataset = pickle.load(open(dataset_path, 'rb'))

        # These are torch tensors, astype means change the data type,
        #.cpu()将数据的处理设备从其他设备（如.cuda()拿到cpu上），不会改变变量类型，
        #.detach()阻断反向传播到此处
